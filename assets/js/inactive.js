// =============================================
// INACTIVE PLAYERS PAGE FUNCTIONS
// =============================================
...
function renderTable(data) {
  ...
  const players = data.players.filter(p => p.status === "Inactive" && p.daysSince >= INACTIVE_THRESHOLD);
  const totalPlayers = players.length;
  const currentPage = tableState.playerTables['Inactive'].page;
  const paginatedPlayers = getPaginatedPlayers(players, currentPage, PLAYERS_PER_PAGE);
  
  if (!players.length) {
    tbl.innerHTML = `<div class='table-container' style='text-align: center; padding: 2rem;'><p>No inactive players found.</p></div>`;
    return;
  }
  ...
  let html = `
    <div class="table-container">
      <div class="table-header">
        <h3 class="table-title">Inactive Players (15+ days) - ${totalPlayers} total</h3>
...
function changePage(page) {
  tableState.playerTables['Inactive'].page = page;
