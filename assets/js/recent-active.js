// =============================================
// RECENT ACTIVE PLAYERS PAGE FUNCTIONS
// =============================================
...
function renderTable(data) {
  ...
  const players = data.recentActivePlayers || [];
  const totalPlayers = players.length;
  const currentPage = tableState.playerTables['RecentActive'].page;
  const paginatedPlayers = getPaginatedPlayers(players, currentPage, PLAYERS_PER_PAGE);
  
  if (!players.length) {
    tbl.innerHTML = `<div class='table-container' style='text-align: center; padding: 2rem;'><p>No recent active players found.</p></div>`;
    return;
  }
  ...
  let html = `
    <div class="table-container">
      <div class="table-header">
        <h3 class="table-title">Recent Active Players - ${totalPlayers} total</h3>
        ...
        <table style="font-size: 0.875rem;">
          <thead>
            <tr>
              ...
              <th style="min-width: 80px;">Gap Days</th>
  ...
  paginatedPlayers.forEach((p, index) => {
    ...
    html += `
      <tr>
        ...
        <td style="text-align: center;">${gapDays}</td>
...
function changePage(page) {
  tableState.playerTables['RecentActive'].page = page;
