...
function renderTable(data) {
  ...
  const currentPage = tableState.playerTables['Active'].page;
  const paginatedPlayers = getPaginatedPlayers(players, currentPage, PLAYERS_PER_PAGE);
  
  if (!players.length) {
    tbl.innerHTML = `<div class='table-container' style='text-align: center; padding: 2rem;'><p>No active players found.</p></div>`;
    return;
  }
  ...
  let html = `
    <div class="table-container">
      <div class="table-header">
        <h3 class="table-title">Active Players (0-2 days) - ${totalPlayers} total</h3>
...
function changePage(page) {
  tableState.playerTables['Active'].page = page;
